...
def _geo_multishape(
    ...
    if "hex" in cube_type.lower():
        cube_geo = """Box(1) = {0, 0, 0, 0.01, 0.01, 0.01};
Transfinite Curve{:} = 3;
Transfinite Surface{:};
Recombine Surface{:};
Transfinite Volume{1};"""
...
MeshSize{ PointsOf{ Volume{1}; } } = 0.006;"""
    ...
    if "tet" in cyl_type.lower():
        cyl_geo = """Cylinder(2) = {0.016, 0, 0.005, 0, 0.012, 0, 0.005, 2*Pi};
...
def define_all_cases() -> list[SimCase]:
    ...
    # Multishape (flat under data/shapes/multishape/)
    multishape_combos = (
        ("hex8_tet4", "HEX8", "TET4", 1, 0),
        ("hex20_tet10", "HEX20", "TET10", 2, 1),
        ("hex27_tet10", "HEX27", "TET10", 2, 0),
        ("tet4_hex8", "TET4", "HEX8", 1, 0),
        ("tet10_hex20", "TET10", "HEX20", 2, 1),
        ("tet10_hex27", "TET10", "HEX27", 2, 0),
    )
    for prefix, cube_t, cyl_t, order, incomp in multishape_combos:
        file_prefix = f"multishape_{prefix}"
        cases.append(
            SimCase(
                shape="multishape",
                elem_type=prefix,
                is_pure_moose=False,
                shape_dir="multishape",
                file_prefix=file_prefix,
                geo_content=_geo_multishape(
                    cube_t, cyl_t, order, incomp
                ),
                moose_content=_moose_gmsh_case(
                    f"{file_prefix}.msh",
                    order_str="SECOND" if order == 2 else "FIRST",
    ...
    # 2D Plate with Hole (flat under data/shapes/platewithhole2d/)
    for etype, (order, incomp) in {
        "QUAD4": (1, 0),
        "QUAD8": (2, 1),
        "QUAD9": (2, 0),
